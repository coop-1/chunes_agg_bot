...
def get_guild_channel_messages(logger, discord_base, discord_header, channel_id, target_date = None):
	...
	try:
		...
		if channel_messages_response.status_code == 200:
			...
			if target_date == None:
				target_date = datetime.now().date() - timedelta(days = 1) ## default to prior day if no parameter is provided
			...
			filtered_tracks = messages_df[
			    ...
			    & (messages_df['date_et'] == target_date)
			    ]['content'].to_list()
